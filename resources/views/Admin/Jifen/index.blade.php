 ...
 <body>
  <div class="mws-panel grid_8"> 
   <div class="mws-panel-header"> 
    <span><i class="icon-table"></i>积分商品列表</span> 
   </div> 
     ...
     <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info"> 
      <thead> 
       <tr role="row">
        ...
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 191px;">积分价格</th>
      ...
      <tbody role="alert" aria-live="polite" aria-relevant="all">
       ...
       <tr class="odd"> 
        ...
        <td class=" ">{{$row->jifen_price}}</td>
        <td class=" "><img src="{{$row->jifen_pic}}" style="max-height: 200px;" alt=""></td> 
        <td class=" ">{{$row->describe}}</td> 
        <td style="text-align: center;">
          <form action="/adminjifen/{{$row->id}}" method="post" style="display: inline">
            {{csrf_field()}}
            {{method_field('DELETE')}}
